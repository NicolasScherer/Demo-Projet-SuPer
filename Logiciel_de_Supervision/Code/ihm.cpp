...
//////partie démo
    //lecteurs
    connect(ui->btLectNew, SIGNAL(clicked()), this, SLOT(newLecteur()));
    connect(ui->btLectDel, SIGNAL(clicked()), this, SLOT(delLecteur()));
    connect(ui->btLectIntrus, SIGNAL(clicked()), this, SLOT(intrusLecteur()));
    //badges
    connect(ui->btBadgeNew, SIGNAL(clicked()), this, SLOT(newBadge()));
}
///////////////////Démo
void Ihm::newLecteur(){
    QString ip = ui->lEditLecteur->text();
    int numLect = ip.mid(13,1).toInt();
    this->lecteurActif(numLect);
}
void Ihm::delLecteur(){
    QString ip = ui->lEditLecteur->text();
    int numLect = ip.mid(13,1).toInt();
    this->lecteurInactif(numLect);
}
void Ihm::intrusLecteur(){
    QString ip = ui->lEditLecteur->text();
    this->lecteurInconnu(ip);
}
void Ihm::newBadge(){
    QString trame = ui->lEditBadge->text();
    this->trameRecu(trame);
}
...
void Ihm::suppLecteur(int numLecteur, int num_vue){
    ...
    //bon QLabel
    QLabel *labelL = pDynamique.labelL[num_vue][numLecteur];
    labelL->deleteLater();

    pBdd->setEtatLect(numLecteur, 0);
 ...
 * Lecteur actif *
 *---------------*/
void Ihm::lecteurActif(int numLecteur){
